...
const CATEGORY_META: Record<string, { emoji: string; label: string }> = {
  ...
  'data-science':            { emoji: '◎',  label: 'Data' },
  'devops':                  { emoji: '⟳',  label: 'DevOps' },
  'ai':                      { emoji: '◆',  label: 'AI' },
  'game-dev':                { emoji: '◉',  label: 'Games' },
  'generic':                 { emoji: '◇',  label: 'Other' },
};

// ── Apple-style minimal SVG icons ────────────────────────────────────────

/** iOS/macOS share sheet icon — box with upward arrow */
function IconShare({ size = 14, className = '' }: { size?: number; className?: string }) {
  return (
    <svg
...
/** Open in new tab — arrow exiting a box */
function IconExternal({ size = 14, className = '' }: { size?: number; className?: string }) {
  return (
    <svg
      width={size} height={size} viewBox="0 0 24 24" fill="none"
      stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"
...
/** Star — slightly heavier for visibility at small sizes */
function IconStar({ size = 14, filled = false }: { size?: number; filled?: boolean }) {
  return (
    <svg
      width={size} height={size} viewBox="0 0 24 24"
      fill={filled ? '#e3b341' : 'none'}
...
/** Fork / branch — two dots connected to a central branch */
...
/** Trending up arrow pill for the page header */
        ...
        <div className="pt-5 pb-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              <style>{`
                .trending-header-icon { color: #60a5fa; }
              `}</style>
              {/* Override color inline since we can't set className on SVG props */}
        ...
        {!loading && !error && (
          <div className="pb-4 -mx-4 md:-mx-6 px-4 md:px-6 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <div className="flex gap-2 w-max">
              ...
              >
                <span style={{ fontSize: '10px' }}>✦</span> All
              </button>
              ...
              {categoryOrder.map((cat) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: '10px', opacity: 0.9 }}>{meta.emoji}</span>
                    {meta.label}
                    <span
              ...
              <div key={category}>
                ...
                {selectedCategory === 'all' && (
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-sm" style={{ color: '#6b7280' }}>{CATEGORY_META[category]?.emoji ?? '◇'}</span>
                ...
                <div className="space-y-2">
                  {repos.map((repo, index) => {
                    ...
                    // Stars growth pill — prefer today, fallback to week
                    const growthCount  = (repo.starsToday && repo.starsToday > 0)
                      ? repo.starsToday
                      : (repo.starsThisWeek && repo.starsThisWeek > 0) ? repo.starsThisWeek : 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        {gs && (
                          <div
                            className="absolute top-3 right-3 flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded leading-none select-none"
                            style={{ background: gs.bg, color: gs.text, border: `1px solid ${gs.border}`, letterSpacing: '0.02em' }}
                          >
                            {repo.healthGrade}
                          </div>
                        )}
                        ...
                        <div className="p-4 flex gap-3">
                          {/* ── Rank badge ── */}
                          <div
                            className="flex-shrink-0 flex items-center justify-center font-bold text-xs tabular-nums select-none"
                            style={{
                              width: '32px',
                              height: '32px',
                              borderRadius: '8px',
                              ...
                              color: index === 0 ? '#fff'
                                   ...
                                   : index === 2 ? '#6b93c4'
                                   : '#4b5563',
                              border: index === 0 ? 'none' : '1px solid #1e2a3a',
                              boxShadow: index === 0 ? '0 0 14px rgba(37,99,235,0.45)' : undefined,
                              marginTop: '1px',
                            }}
                          >
                            {index + 1}
                          ...
                          {/* ── Body ── */}
                          <div className="flex-1 min-w-0" style={{ paddingRight: gs ? '52px' : '8px' }}>
                            ...
                            <div className="flex items-center justify-between gap-2 flex-wrap">
                              ...
                              {/* Stars + growth + action icons */}
                              <div className="flex items-center gap-2 flex-shrink-0">
                                {/* Stars */}
                                <div className="flex items-center gap-1">
                                  <IconStar size={13} filled />
                                  <span className="text-xs font-semibold tabular-nums" style={{ color: '#e3b341' }}>
                                    {fmtNum(repo.stars)}
                                  </span>
                                </div>
