  ...
  if (!loading && !connection) {
    return (
      ...
      >
        ...
        {/* Reset stale connection — shown for users stuck in a broken state */}
        <button
          onClick={async () => {
            setResetting(true);
            await disconnect();
          ...
          disabled={resetting}
          className="w-full mt-2 py-1.5 rounded-xl text-xs transition-opacity"
          style={{ background: 'transparent', color: '#4b5563', border: '1px solid #21262d' }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#8b949e'; }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#4b5563'; }}
        >
          {resetting ? 'Clearing…' : 'Already connected but not showing? Reset & reconnect'}
        </button>
      </div>
    );
  }
