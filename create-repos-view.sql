...
SELECT 
  ...
  r.description,
  r.owner_login,
  r.avatar_url as owner_avatar,  -- Map avatar_url to owner_avatar for app compatibility
  ...
  COALESCE(
    (SELECT trending_score 
     ...
     WHERE repo_id = r.repo_id 
       ...
       AND period_date = (SELECT MAX(period_date) FROM trending_repos WHERE period_type = 'daily')
     LIMIT 1),
    0.0
  ) as trending_score,
  
  -- Star velocity from trending_repos table
  COALESCE(
    (SELECT star_velocity 
     FROM trending_repos 
     WHERE repo_id = r.repo_id 
       AND period_type = 'daily'
       AND period_date = (SELECT MAX(period_date) FROM trending_repos WHERE period_type = 'daily')
  ...
  -- Badges from repo_badges table
  COALESCE(
    (SELECT array_agg(badge_slug) 
     FROM repo_badges 
     WHERE repo_id = r.repo_id),
    ARRAY[]::TEXT[]
  ) as badges,
  
  -- Gem status from repo_gems table
  CASE 
    WHEN EXISTS (SELECT 1 FROM repo_gems WHERE repo_id = r.repo_id) 
    THEN true 
    ELSE false 
  END as is_gem,
  
  -- Gem score (if available)
  COALESCE(
    (SELECT gem_score FROM repo_gems WHERE repo_id = r.repo_id),
    0.0
  ) as gem_score,
